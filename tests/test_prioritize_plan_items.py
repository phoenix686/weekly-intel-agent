...
def test_stale_nudge_entry_with_no_new_item():
    """A card can be surfaced with source=stale_nudge and no corresponding
    matched item this week -- purely from board staleness."""
    cards = [_card("card1")]
    reply = [{"matched_card_id": "card1", "source": "stale_nudge", "item_url": None,
    ...
    assert result["prioritized_project_work"][0]["source"] == "stale_nudge"
    assert result["prioritized_project_work"][0]["item_url"] is None
...
def test_hallucinated_card_id_dropped():
    cards = [_card("card1")]
    reply = [
        {"matched_card_id": "card1", "source": "stale_nudge", "item_url": None, "priority_reasoning": "r", "movement_note": None},
...
def test_order_preserved_from_model_response():
    ...
    reply = [
        ...
        {"matched_card_id": "card1", "source": "stale_nudge", "item_url": None, "priority_reasoning": "lower priority", "movement_note": None},
    ]
...
def test_movement_block_included_in_prompt():
    ...
    with patch.object(prioritize_mod.client.messages, "create", return_value=_haiku_response(reply)) as mock_create, \
         patch.object(prioritize_mod, "record_node_summary"):
        prioritize_plan_items(_state([], cards, card_movements=movements))
...
def test_course_tagged_items_excluded_from_candidates():
    items = [_matched_item("https://a.com/1", "card1", tags=["course"])]
    cards = [_card("card1")]

    with patch.object(prioritize_mod.client.messages, "create", return_value=_haiku_response([])) as mock_create, \
         patch.object(prioritize_mod, "record_node_summary"):
