...
def test_within_run_near_duplicates_keep_fuller_text():
    a = _item("https://a.com/1", "Story A", "short version")
    b = _item("https://b.com/1", "Story A elsewhere", "much longer, fuller version of the same story")
    ...
    with patch("discovery.semantic_dedup.get_store", return_value=fake_store), \
        ...
        survivors, costs = dedupe_semantic([a, b])

    assert len(survivors) == 1
    assert survivors[0]["url"] == "https://b.com/1"  # fuller text won
    drop_errors = [c["error"] for c in costs if c.get("error")]
    assert any("kept over near-duplicate" in e and "a.com" in e for e in drop_errors)
...
def test_cross_run_match_drops_new_item_unconditionally():
    ...
    with patch("discovery.semantic_dedup.get_store", return_value=fake_store), \
         patch("discovery.semantic_dedup.embed_text", side_effect=_embed_side_effect(vectors)):
        survivors, costs = dedupe_semantic([new_item])
...
def test_window_entries_older_than_7_days_are_excluded_and_deleted():
    ...
    with patch("discovery.semantic_dedup.get_store", return_value=fake_store), \
         patch("discovery.semantic_dedup.embed_text", side_effect=_embed_side_effect(vectors)):
        survivors, costs = dedupe_semantic([item])
