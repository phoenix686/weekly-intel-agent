...
at least 2 entries present, and every feed_url entry returns real content
on a live fetch (not assumed from a prior commit).
"""
import sys
import os
...
def test_every_entry_has_name_and_exactly_one_url_key():
    ...
    for entry in entries:
        ...
        has_feed = "feed_url" in entry
        has_scrape = "scrape_url" in entry
        assert has_feed != has_scrape, (
            f"{entry['name']!r} must have exactly one of feed_url/scrape_url, got: {entry}"
        )
...
def test_feed_url_entries_return_real_content_live(entry):
    if "feed_url" not in entry:
        pytest.skip(f"{entry['name']} uses scrape_url, not feed_url")
